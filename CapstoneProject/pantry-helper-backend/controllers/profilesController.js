...
exports.addProfile = (req, res) => {
  ...
  const query = `
    ...
    ON DUPLICATE KEY UPDATE
      ...
      currentPantry = VALUES(currentPantry)
  `
  ...
  pool.query(
    ...
    (error, results) => {
      if (error) {
        ...
        return res.status(500).json({ error: 'Error adding profile' })
      }

      if (results.insertId === 0) {
        const selectQuery = 'SELECT profileID FROM Profiles WHERE email = ?'
        pool.query(selectQuery, [email], (err, selectResults) => {
          if (err) {
            console.error('Error retrieving profile ID:', err)
            return res
              .status(500)
              .json({ error: 'Error retrieving profile ID' })
          }
          res.status(200).json({
            message: 'Profile updated successfully',
            profileID: selectResults[0].profileID
          })
        })
      } else {
        res.status(201).json({
