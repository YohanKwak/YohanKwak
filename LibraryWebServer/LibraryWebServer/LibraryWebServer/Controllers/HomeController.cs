...
namespace LibraryWebServer.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult CheckLogin(string name, int cardnum)
        {
            ...
            var query = from p in db.Patrons where p.Name == name select new { patronCard = p.CardNum };
            foreach (var x in query)
            {
                if (x.patronCard == cardnum)
                {
                    user = name;
                    card = cardnum;
        ...
        public ActionResult AllTitles()
        {
            var query = from t in db.Titles
                        ...
                        on t.Isbn equals i.Isbn into titleInventory
                        from ti in titleInventory.DefaultIfEmpty()
                        join c in db.CheckedOut
                        on ti.Serial equals c.Serial into titleInventoryCheckOut
                        from tic in titleInventoryCheckOut.DefaultIfEmpty()
                        join p in db.Patrons on tic.CardNum equals p.CardNum into fin
                        from f in fin.DefaultIfEmpty()
                        select new { isbn = t.Isbn, title = t.Title, author = t.Author, serial = ti.Serial.Equals(null) ? null : (uint?)ti.Serial, name = f.Name == null ? "" : f.Name };
        ...
        public ActionResult ListMyBooks()
        {
            var query = from c in db.CheckedOut
                        ...
                        into SerialofCheckedOut
                        from sc in SerialofCheckedOut.DefaultIfEmpty()
                        join t in db.Titles
                        on sc.Isbn equals t.Isbn into fin
                        from f in fin.DefaultIfEmpty()
                        where c.CardNum == card
                        select new { title = f.Title, author = f.Author, serial = sc.Serial };
        ...
        public ActionResult CheckOutBook(int serial)
        {
            ...
            newCheckedOut.Serial = (uint)serial;
            newCheckedOut.CardNum = (uint)card;

            db.CheckedOut.Add(newCheckedOut);
